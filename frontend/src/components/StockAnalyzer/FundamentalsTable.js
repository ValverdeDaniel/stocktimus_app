      ...
      <table className="w-full text-xs min-w-max">
        <thead>
          <tr className="border-b border-gray-700">
            {/* Stock Info */}
            <th className="text-left py-2 px-2 text-gray-300 sticky left-0 bg-gray-900 z-10">Ticker</th>
            <th className="text-right py-2 px-2 text-gray-300 cursor-pointer hover:text-white" onClick={() => handleSort('Current Price')}>
              Current<br/>Price {sortField === 'Current Price' && (sortDirection === 'asc' ? '↑' : '↓')}
            </th>

            {/* Price Highs */}
            <th className="text-right py-2 px-2 text-gray-300 cursor-pointer hover:text-white" onClick={() => handleSort('All-Time High')}>
              All-Time<br/>High {sortField === 'All-Time High' && (sortDirection === 'asc' ? '↑' : '↓')}
            </th>
            <th className="text-right py-2 px-2 text-gray-300 cursor-pointer hover:text-white" onClick={() => handleSort('ATH %Chg')}>
              ATH<br/>%Chg {sortField === 'ATH %Chg' && (sortDirection === 'asc' ? '↑' : '↓')}
            </th>
            <th className="text-right py-2 px-2 text-gray-300 cursor-pointer hover:text-white" onClick={() => handleSort('6-Month High')}>
              6-Month<br/>High {sortField === '6-Month High' && (sortDirection === 'asc' ? '↑' : '↓')}
            </th>
            <th className="text-right py-2 px-2 text-gray-300 cursor-pointer hover:text-white" onClick={() => handleSort('Recent-High %Chg')}>
              Recent-High<br/>%Chg {sortField === 'Recent-High %Chg' && (sortDirection === 'asc' ? '↑' : '↓')}
            </th>
            <th className="text-right py-2 px-2 text-gray-300 cursor-pointer hover:text-white" onClick={() => handleSort('5-Year High')}>
              5-Year<br/>High {sortField === '5-Year High' && (sortDirection === 'asc' ? '↑' : '↓')}
            </th>

            {/* P/E Metrics */}
            <th className="text-right py-2 px-2 text-gray-300 cursor-pointer hover:text-white" onClick={() => handleSort('Current P/E')}>
              Current P/E {sortField === 'Current P/E' && (sortDirection === 'asc' ? '↑' : '↓')}
            </th>
            <th className="text-right py-2 px-2 text-gray-300 cursor-pointer hover:text-white" onClick={() => handleSort('Avg 5-Year P/E')}>
              Avg 5-Year<br/>P/E {sortField === 'Avg 5-Year P/E' && (sortDirection === 'asc' ? '↑' : '↓')}
            </th>

            {/* Fair Value */}
            <th className="text-right py-2 px-2 text-gray-300 cursor-pointer hover:text-white" onClick={() => handleSort('Fair Value (TTM)')}>
              Fair Value<br/>(TTM) {sortField === 'Fair Value (TTM)' && (sortDirection === 'asc' ? '↑' : '↓')}
            </th>
            <th className="text-right py-2 px-2 text-gray-300 cursor-pointer hover:text-white" onClick={() => handleSort('Fair Value %Chg')}>
              Fair Value<br/>%Chg {sortField === 'Fair Value %Chg' && (sortDirection === 'asc' ? '↑' : '↓')}
            </th>
            <th className="text-right py-2 px-2 text-gray-300 cursor-pointer hover:text-white" onClick={() => handleSort('Avg P/E × Fwd EPS')}>
              Avg P/E ×<br/>Fwd EPS {sortField === 'Avg P/E × Fwd EPS' && (sortDirection === 'asc' ? '↑' : '↓')}
            </th>

            {/* Technical Indicators */}
            <th className="text-right py-2 px-2 text-gray-300 cursor-pointer hover:text-white" onClick={() => handleSort('MACD (12,26,9)')}>
              MACD<br/>(12,26,9) {sortField === 'MACD (12,26,9)' && (sortDirection === 'asc' ? '↑' : '↓')}
            </th>
            <th className="text-right py-2 px-2 text-gray-300 cursor-pointer hover:text-white" onClick={() => handleSort('MACD Signal')}>
              MACD<br/>Signal {sortField === 'MACD Signal' && (sortDirection === 'asc' ? '↑' : '↓')}
            </th>
            <th className="text-right py-2 px-2 text-gray-300 cursor-pointer hover:text-white" onClick={() => handleSort('RSI (1 yr)')}>
              RSI (1 yr) {sortField === 'RSI (1 yr)' && (sortDirection === 'asc' ? '↑' : '↓')}
            </th>

            {/* Time */}
            <th className="text-center py-2 px-2 text-gray-300">
              Live Price<br/>Time
            </th>

            {/* Actions */}
            <th className="text-center py-2 px-2 text-gray-300 sticky right-0 bg-gray-900 z-10">Actions</th>
          </tr>
        </thead>
        <tbody>
          {sortedData.map((row, index) => (
            <tr key={index} className="border-b border-gray-800 hover:bg-gray-700/30">
              {/* Stock Info */}
              <td className="py-2 px-2 font-medium text-white sticky left-0 bg-gray-900">
              ...
              {/* Actions */}
              <td className="py-2 px-2 text-center sticky right-0 bg-gray-900">
