...
class RunScreenerAPIView(APIView):
    ...
    def post(self, request):
        try:
            ...
            print("[DEBUG] Incoming param_sets:", param_sets)
...
class RunWatchlistAPIView(APIView):
    ...
    def post(self, request):
        try:
            ...
            print("[DEBUG] Incoming watchlist contracts (processed):", contracts_input)

            # --- Run simulation ---
            df = whole_watchlist(contracts_input)

            if df.empty:
                print("[WARNING] Simulation returned empty DataFrame — likely no contracts matched.")
                return Response([
                    {
                        "Ticker": "N/A",
            ...
            for result in results:
                ...
                if not original_contract:
                    print(f"[WARNING] No matching original contract for result: {result_key}")
                    continue
            ...
            print(f"[SUCCESS] Returning {len(cleaned_data)} simulated contract(s).")
            return Response(cleaned_data, status=status.HTTP_200_OK)
...
class SavedContractListCreateAPIView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            initial_equity = number_of_contracts * avg_cost
            ...
            serializer.save(
                ...
                current_equity_value=initial_equity
            )
        except Exception as e:
...
def refresh_contract_data(request, contract_id):
    try:
        ...
        df = whole_watchlist([contract_data])
        if not df.empty:
            result = df.iloc[0]
            
            # Update current values
            current_premium = result.get("Current Premium", contract.current_premium)
            current_underlying = result.get("Underlying Price", contract.current_underlying_price)
            
            if current_premium and not math.isnan(current_premium):
                contract.current_premium = current_premium
                contract.current_equity_value = contract.number_of_contracts * current_premium
        ...
        return Response({"error": f"Refresh failed: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def post(self, request, group_id):
        try:
            ...
            for result in results:
                ...
                # Update contract record with current values from simulation
                if contract:
                    current_premium = result.get("Current Premium")
                    current_underlying = result.get("Underlying Price")
                    
                    if current_premium and not math.isnan(current_premium):
                        contract.current_premium = current_premium
                        contract.current_equity_value = num_contracts * current_premium
                    
                    if current_underlying and not math.isnan(current_underlying):
                        contract.current_underlying_price = current_underlying
                    
                    # Store simulation result for reference
                    contract.latest_simulation_result = result
                    contract.last_refresh_date = timezone.now()
                    contract.save()
