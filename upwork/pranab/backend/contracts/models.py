...
class SavedContract(models.Model):
    ...
    initial_equity_invested = models.FloatField(null=True, blank=True)
    current_equity_value = models.FloatField(null=True, blank=True)
    ...
    def days_until_expiration(self):
        """Calculate days remaining until option expiration"""
        if not self.expiration:
            return 0
        today = timezone.now().date()
        return max(0, (self.expiration - today).days)
    
    def underlying_percent_change(self):
        """Calculate percentage change in underlying price"""
        if not self.underlying_price_at_add or not self.current_underlying_price:
            return 0
        if self.underlying_price_at_add == 0:
            return 0
    ...
    def premium_percent_change(self):
        ...
        if not self.initial_premium or not self.current_premium:
            return 0
    ...
    def equity_percent_change(self):
        """Calculate percentage change in total equity value"""
        if not self.initial_equity_invested or not self.current_equity_value:
            return 0
        if self.initial_equity_invested == 0:
            return 0
        return ((self.current_equity_value - self.initial_equity_invested) / self.initial_equity_invested) * 100
